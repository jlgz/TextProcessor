...
void DFS_save(FILE *fp,Node *x,int nfich){
  char *st;
  int stSize;
  if (x->right != NIL)
    DFS_save(fp,x->right,nfich);

  if (x->left != NIL)
    DFS_save(fp,x->left,nfich);
  st = (x->data)->key;
  stSize = strlen(st);
  fwrite(&stSize,sizeof(int),1,fp);
  fwrite(st,sizeof(char),stSize,fp);
  fwrite(&((x->data)->num),sizeof(int),1,fp);
  fwrite((x->data)->vector,sizeof(int),nfich,fp);
}
void initDFS_save(char *filename,RBTree *tree,int nfich){
  FILE *fp = fopen(filename, "w");
  int nNods = getNumNodes(tree);
  fwrite(&nNods, sizeof(int),1,fp);
  fwrite(&nfich,sizeof(int),1,fp);
  if (tree->root != NIL)
    DFS_save(fp, tree->root,nfich);
  fclose(fp);
}
int  load_tree(char *filename, RBTree *tree){
	initTree(tree);
	int nNods,nFich,sz,cont =0;
	RBData *treeData;
	deleteTree(tree);
	initTree(tree);
	FILE *fp = fopen(filename, "r");
	fread(&nNods, sizeof(int),1,fp);
  	fread(&nFich,sizeof(int),1,fp);
	while(cont< nNods){
		cont++;
		fread(&sz, sizeof(int),1,fp);
		treeData = malloc(sizeof(RBData));
      		treeData->key = malloc(sizeof(char) * 100);
      		fread(treeData->key, sizeof(char),sz,fp); 
		treeData->key[sz] = 0;
      		fread(&(treeData->num), sizeof(int),1,fp);
      		treeData->vector = malloc(sizeof(int)* nFich);
		fread(treeData->vector, sizeof(int),nFich,fp);
		//printf(" %s %d \n",treeData->key,cont);
